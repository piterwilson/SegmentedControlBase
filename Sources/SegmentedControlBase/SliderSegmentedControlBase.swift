...
import SwiftUI
/**
`SliderSegmentedControlBase` is a base view that can be used to construct custom segment control components.
...
The component has properties to configure the values for the segments (`segments`) as well as a two way binding to the control's selected segment index (`selectedSegmentIndex`).
 
The views used to display each segment are specified with the `segmentBuilder` view builder. This view builder is passed three parameters: the first parameter passed is a `String` contaning a value for the segment being displayed.  the second parameter is a `Bool` indicating whether the segment is selected or not and the third parameter is a `Bool` indicating whether or not the control is enabled.
